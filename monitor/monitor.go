...
type Monitor struct {
    ctl_host string
    hostnames []string
    tree_head ctl_parsing.Signed_tree_head
    database *sql.DB
...
func NewMonitor(ctl_host string, hostnames []string, verbose bool, no_delete bool, non_strict bool) (*Monitor, error) {
    ...
    sth, err := ctl_parsing.Get_sth(ctl_host)
    if err != nil {
...
// return tree head
func (m *Monitor) Get_tree_head() ctl_parsing.Signed_tree_head {
...
// prepare a statement to insert results into the database
    ...
    var entries []ctl_parsing.Raw_entry
    var leaf ctl_parsing.Merkle_tree_leaf
...
// parse the certificate entry and extract the commonname field.  if it's malformed, skip it and go on to the next entry
            common_name, err = ctl_parsing.Get_commonname(leaf)
            if err != nil {
                log.Println(err)
                continue
            }
            if m.VERBOSE { fmt.Println(timestamp, common_name, ctl_parsing.LOG_ENTRY_TYPE_MAP[leaf.LogEntryType]) }
...
// if it is, add the timestamp, certificate type, commonname, and certificate type to the database, and increment the appropriate metric
            if i != -1 {
                if m.VERBOSE { fmt.Println("Adding", timestamp, common_name, leaf.Entry, ctl_parsing.LOG_ENTRY_TYPE_MAP[leaf.LogEntryType]) }
                results, _ := statement.Exec(timestamp, common_name, leaf.Entry, ctl_parsing.LOG_ENTRY_TYPE_MAP[leaf.LogEntryType])
                rows_added, _ := results.RowsAffected()

                metric := m.certificate_metrics.WithLabelValues(hostname, ctl_parsing.LOG_ENTRY_TYPE_MAP[leaf.LogEntryType])
...
// get the new signed tree head; if there's a problem, print and error and return
    new_sth, err := ctl_parsing.Get_sth(m.ctl_host)
    if err != nil {
